...
#%% Parquet Data
base_path = "C:/Users/ayedd/Dropbox/LinkedIn/raw_data/"
folder1 = "sample_pos_1pc.parquet/"
...
#%% city data
add_path_to_cm = "C:/Users/ayedd/Dropbox/merged_city_mapper.csv"
city_mapper_merged = dd.read_csv(add_path_to_cm)
...
cluster = LocalCluster(n_workers=1, threads_per_worker=2, memory_limit='30 GB')
client = Client(cluster)
client.dashboard_link
...
@dask.delayed(nout = 0)
def process_files(root_path, parquet_folder, parquet_file, df_concat_loc):
    """
    Parameters
    ----------
    parquet_file : path
        Path from list of paths to the individual parquet files
    df_concat_loc : concatenated city mapper file
    ...
    ddf = test_parquet.merge(df_concat_loc, left_on = ['location_raw', 'metro_area'], right_on = ['location', 'metro_area'], how = 'left')
    ...
    print(f"Merge Elapsed Time: {time_elapsed:.2f} seconds", flush= True)

    start_time = time.time()
    parquet_city_merged = ddf_rows_with_location.compute()
    #parquet_city_merged.to_parquet(root_path + 'akm_parquet/' + parquet_file.split(".")[0] + '_' + parquet_file.split(".")[1] + '_test.parquet')
    end_time = time.time()
    time_elapsed = end_time - start_time

    start_time = time.time()
    dropped_ids = ddf_rows_without_location['user_id'].compute()
...
#tasks = [process_files(base_path, folder1, parquet_file, city_mapper_merged) for parquet_file in parquet_files]
task = process_files(base_path, folder1, parquet_files[0], city_mapper_merged)
...
print(f"Elapsed Time: {time_elapsed:.2f} seconds", flush= True)
